...
test("ships configurable privacy-safe map visualizations",async()=>{const[page,css,api]=await Promise.all([readFile(new URL("../app/page.tsx",import.meta.url),"utf8"),readFile(new URL("../app/iteration.css",import.meta.url),"utf8"),readFile(new URL("../api/server.mjs",import.meta.url),"utf8")]);for(const view of ["category","splatter","weather","hexbin","dominance","clusters"]){assert.match(page,new RegExp(`${view}:`));assert.match(api,new RegExp(`\\b${view}\\b`))}assert.match(page,/Default map visualization/);assert.match(page,/reduced_map_motion/);assert.match(page,/colorblind_map/);assert.match(css,/\.map-view-selector/);assert.match(css,/\.view-weather/);assert.match(css,/\.view-hexbin/);assert.match(css,/\.view-clusters/)});
