...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "PATCH" && url.pathname === "/api/profile") {
      ...
      await supabase(`/rest/v1/profiles?id=eq.${user.id}`, { method:"PATCH",data:update,prefer:"return=minimal" });return json(res,200,{...update},origin);
    }
    ...
    if(req.method==="GET"&&url.pathname==="/api/map/heatmap"){
      rateLimit(req,120);const mapConfig=await configValue("map_layers",{minimum_heat_reports:3,grid_degrees:4});const days=Math.min(90,Math.max(1,Number(url.searchParams.get("days"))||7));const rows=await supabase("/rest/v1/rpc/activity_heatmap",{method:"POST",token:ANON_KEY,data:{p_since:new Date(Date.now()-days*86400000).toISOString(),p_grid_degrees:Math.max(2,Number(mapConfig.grid_degrees)||4),p_minimum:Math.max(3,Number(mapConfig.minimum_heat_reports)||3)}});return json(res,200,{cells:rows},origin);
    }
    ...
    if (req.method === "POST" && url.pathname === "/api/sightings") {
      ...
      const rows = await supabase("/rest/v1/sightings", { method: "POST", data: {
        ...
        notes: typeof input.notes === "string" ? input.notes.trim().slice(0, 1000) || null : null,weather,reporter_attribution:attribution,
    ...
    if (req.method === "POST" && confirmation) {
      rateLimit(req, 30);
      const { user } = await activeUser(req);
      await supabase("/rest/v1/confirmations?on_conflict=sighting_id,hunter_id", { method: "POST", data: { sighting_id: confirmation[1], hunter_id: user.id }, prefer: "resolution=ignore-duplicates,return=minimal" });const owners=await supabase(`/rest/v1/sightings?id=eq.${confirmation[1]}&select=reporter_id`);if(owners[0]?.reporter_id!==user.id)await notify(owners[0]?.reporter_id,"confirmation","Activity confirmed","Another member confirmed your bird activity.",`/?sighting=${confirmation[1]}`);
      return json(res, 204, null, origin);
    }
    ...
    if(req.method==="GET"&&url.pathname==="/api/admin/moderation"){
      await requireRole(req,["moderator","admin"]);const status=url.searchParams.get("status")||"open";const allowed=["open","assigned","escalated","approved","removed","duplicate","needs_info"];if(!allowed.includes(status))throw Object.assign(new Error("Invalid queue"),{status:400});const rows=await supabase(`/rest/v1/moderation_cases?status=eq.${status}&select=*&order=created_at.desc&limit=200`);return json(res,200,{cases:rows},origin);
    }
