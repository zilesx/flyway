...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if(req.method==="GET"&&adminUserActivity){const{user}=await requireRole(req,["admin"]);const page=Math.max(1,Number(url.searchParams.get("page"))||1),limit=50,offset=(page-1)*limit;const rows=await supabase(`/rest/v1/user_activity_log?user_id=eq.${adminUserActivity[1]}&select=id,user_id,action,target_type,target_id,outcome,before_state,after_state,request_context,session_id,created_at&order=created_at.desc&offset=${offset}&limit=${limit+1}`);await auditRequest(req,user.id,"user_activity.view","profile",adminUserActivity[1],{page});return json(res,200,{events:rows.slice(0,limit),page,has_more:rows.length>limit},origin)}
